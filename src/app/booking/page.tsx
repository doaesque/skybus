...
import { ArrowLeft, User, Armchair, CheckCircle, Info, X, Baby, Users, CalendarRange } from 'lucide-react';
...
function BookingContent() {
  ...
  const totalPrice = selectedSeats.length * basePrice * multiplier;
      ...
      return (
          <div className="bg-slate-100 dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className={`mx-auto bg-slate-300 dark:bg-slate-700 rounded-lg mb-8 p-3 shadow-inner flex justify-between items-center px-4 ${mapWidth}`}>
                  {layout === "shuttle" ? (
                      <div className="flex items-center gap-2">{renderSeat('1A')}</div>
                  ) : (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 font-sans">
        ...
        {/* Ini dipisah dari grid agar sejajar di atas */}
        <div className="flex items-center gap-4 mb-6">
            <Link href="/ticket" className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 transition">
                ...
                <div className="sticky top-24 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-lg space-y-5">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium">
                                {paxCount} Dewasa
                                {hasInfant && <span className="text-pink-500 font-bold ml-1">+ Bayi</span>}
                            </p>
                        </div>
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-sm font-medium">Rp {basePrice.toLocaleString('id-ID')}</span>
                        </div>
                        {isRoundTrip && (
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-xs font-bold text-blue-600 flex items-center gap-1"><CalendarRange className="w-3 h-3"/> Round Trip</span>
                                <span className="text-xs font-bold text-blue-600">x 2 Perjalanan</span>
                            </div>
                        )}
