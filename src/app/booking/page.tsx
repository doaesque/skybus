...
import { ArrowLeft, User, Armchair, CheckCircle, Info, X, Baby, Users, CalendarRange, Clock, ChevronRight, AlertTriangle } from 'lucide-react';
...
function BookingContent() {
  ...
  // --- STATE ---
  const [step, setStep] = useState(1); // 1: Kursi, 2: Data
  ...
  // Adjust infant array size when count changes
  useEffect(() => {
      setInfants(prev => {
  ...
  const handleInfantChange = (index: number, field: string, val: string) => {
    ...
    // @ts-ignore
    newInfants[index] = { ...newInfants[index], [field]: val };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 font-sans">
        ...
        {/* --- MAIN GRID --- */}
        {/* Hapus items-start agar grid behavior default (stretch) bekerja untuk sticky */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="w-full md:w-96 shrink-0">
                {/* Sticky Wrapper - top-24 assumes navbar height ~80px */}
